...
def edit_shelter_animal(animal_id):
    ...
    # Select target animal's shelter ID
    query = """
        SELECT shelter_id FROM Animals
        WHERE animal_id = %s
    """
    cursor.execute(query, (animal_id,))
    target_shelter_id = cursor.fetchone()

    # Stop if animal is not associated with current shelter login
    if target_shelter_id[0] != session.get('shelter_id', -999):
